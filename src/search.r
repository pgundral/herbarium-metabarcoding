...
# Read in BRU data
occurrences <- read.csv("./data/raw/BrownUniversity-BRU_DwC-A/occurrences.csv")

# Read in genera identified by ITIS processor 
genera <- read.csv("data/processed/genera.csv")
...
write.csv(specimens, file='./data/processed/specimens.csv')
write.csv(sliced, file='./data/processed/sliced_specimens.csv')
...
yellowstone <- read_excel("data/raw/NPSpecies_Checklist_YELL_20250223195402.xlsx")
...
write.csv(yellowstone_specimens, file='./data/processed/yellowstone_specimens.csv')
write.csv(sliced_yellowstone_specimens, file='./data/processed/yellowstone_specimens_sliced.csv')
write(paste(yellowstone_specimens$occurrenceID, collapse = ', '), './data/processed/yellowstone_specimen_ids.txt')

# write other specimen IDs to CSV
temp <- specimens$occurrenceID[!(specimens$occurrenceID %in% yellowstone_specimens$occurrenceID)]
write(paste(temp, collapse = ', '), './data/processed/other_specimen_ids.txt')
